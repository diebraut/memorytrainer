...
PackageDesc::PackageDesc(QString packageName,QObject *parent)
    : QObject(parent),
...
QList<EntryDesc *> PackageDesc::getEntriesForList(QList<EntryDesc *> listEntries,int idxPackageList) {

    QList<EntryDesc *> returnList;
    if (this->showList == DISPLAY_ALL) {
        listEntries = this->xmlParser->getExercizeList("MainÜbungsliste",idxPackageList,listEntries);
        listEntries = this->xmlParser->getExercizeList("MainÜbungslisteUmgekehrt",idxPackageList,listEntries);
